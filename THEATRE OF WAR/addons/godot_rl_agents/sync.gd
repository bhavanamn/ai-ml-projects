...
func _ready():
	print("sync file intially called")
...
func _initialize_training_agents():
	if agents_training.size() > 0:
		...
		print('No training agents. Something wrong. the call "agents_training.size()" returns 0')
		print('Info:Agents:', agents_training)
...
func _initialize_inference_agents():
	if agents_inference.size() > 0:
		...
		for agent in agents_inference:
			...
			print("onnx model path",agent_onnx_model)
...
func _training_process():
	if connected:
		...
		if need_to_send_obs:
			...
			#print("reward from sync file",reward)
...
#func _set_agent_mode(agent: Node):
	#print("set agent mode function called")
	##print("need to set the agent modes, will be requiring control modes")
	## Use AIController3D.ControlModes to avoid ambiguity
	#var agent_inherits_mode: bool = agent.control_mode == AIController3D.ControlModes.INHERIT_FROM_SYNC
	#print('agent_inherits_mode', agent_inherits_mode, control_mode)
	#
	#if agent_inherits_mode:
		#match control_mode:
			#AIController3D.ControlModes.HUMAN:
				#agent.control_mode = AIController3D.ControlModes.HUMAN
			#AIController3D.ControlModes.TRAINING:
				#agent.control_mode = AIController3D.ControlModes.TRAINING
			#AIController3D.ControlModes.ONNX_INFERENCE:
				#agent.control_mode = AIController3D.ControlModes.ONNX_INFERENCE

func _get_agents():
	#print("get agents intialized")
	var all_agents = get_tree().get_nodes_in_group("AGENT")
	print("printing all agents in sync.gd file",all_agents)
	for agent in all_agents:
		print(agent.control_mode)
...
#        _send_dict_as_json_message(reply)
	...
	if message["type"] == "call":
		...
		_send_dict_as_json_message(reply)
		print("msg type from sync",message["method"],reply)
