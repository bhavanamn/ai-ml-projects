...
# entry point
static func generate_paths(path_points, caller_node, color, offset_x, offset_y, c_l, c_m, width = null):
	var road_width = 2 if not width else width
	var path_polygon = [
		Vector2(-(road_width / 2), 0.03),
		Vector2(-(road_width / 2), 0.03),
		Vector2(road_width / 2, 0.03),
		Vector2(road_width / 2, 0.03)
	]
	...
	for path in path_points:
		...
		#print("polygon.collision_layer ",polygon.collision_layer )
